...
class SAND():
    ...
    def fit(self, X, y=None, online=False, alpha=None, init_length=None, batch_size=None, overlaping_rate=1, #overlaping_rate=10,
            verbose=False):
        ...
        if online:
            ...
            while self.current_time < len(self.ts) - self.subsequence_length:
                ...
                if self.current_time < len(self.ts) - self.subsequence_length:
                    self.decision_scores_ += self._run(
                        self.ts[self.current_time - self.batch_size:min(len(self.ts), self.current_time)])
                else:
    ...
    def _run(self, ts):
        ...
        for sub_join, scores_sub_join, scores_sub_join_old, t_decay in zip(all_join, self.weights,
    ...
    def _kshape_subsequence(self, initialization=True):
        ...
        for i in range(self.current_time,
                       min(self.current_time + nb_subsequence, len(self.ts) - self.subsequence_length),
